module "cloudfront" {
  ...
  s3_origin_domain_name = "${module.s3_react.react_bucket_regional_domain_name}"
  s3_origin_id          = "${module.s3_react.react_bucket_id}"
  domain_aliases        = ["${"${terraform.workspace}" == "prod" ?  "mxfactorial.io" : "${terraform.workspace}.mxfactorial.io"}"]
  ssl_arn               = "${lookup("${null_resource.client_cert_arns.triggers}", "${terraform.workspace}")}"
}
