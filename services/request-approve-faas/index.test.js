...
const compareRequest = require('./src/compareRequest')
...
describe('transact function handler', () => {
  // test position avoids multiple mockImplementationOnce for compareRequest
  ...
  it('calls compareRequest with request', async () => {
    await handler({ items: itemsStandardArray })
    expect(compareRequest).toHaveBeenCalledWith(
      itemsStandardArray,
      itemsStandardArray
    )
  })
