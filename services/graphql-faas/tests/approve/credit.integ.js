...
describe('graphql approve credit request', () => {
  it('credit approval request values returned', async () => {
    // debitor creates credit request
    let createRequestResponse = await debitorGraphQLClient
      .request(createRequest, {
        items: creditRequest
      })
    // creditor approves debit request
    let approveRequestResponse = await creditorGraphQLClient
      .request(approveRequest, {
        items: createRequestResponse.createRequest
      })
    expect(approveRequestResponse.approveRequest)
      .toHaveLength(creditRequest.length)
    expect(approveRequestResponse.approveRequest[0].debitor)
      .toBe(creditRequest[0].debitor)
