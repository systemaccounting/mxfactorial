...
test('JacobWebb transacts with GroceryStore', async ({ page }) => {
	...
	const JacobWebbAccountBalance = await secondPage
		.locator('css=[data-id="accountBalance"]')
		.innerText();

	await page.locator('css=[data-id="homeIcon"]').click();
	await page.waitForTimeout(500); // wait for new value in balance component
	const GroceryStoreAccountBalance = await page
		.locator('css=[data-id="accountBalance"]')
		.innerText();
...
async function getFirstNTransactionItems(page, n) {
	...
	for (let i = 0; i < n; i++) {
		transactionItems.push({
			...
			quantity: await page
				.locator(`css=[data-id-index="${i}"] .transaction-item-quantity`)
				.innerText(),
