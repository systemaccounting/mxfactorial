...
resource "aws_route53_record" "client_www_cert_validation" {
  name    = tolist(aws_acm_certificate.client_www_cert.domain_validation_options)[0].resource_record_name
  type    = tolist(aws_acm_certificate.client_www_cert.domain_validation_options)[0].resource_record_type
  zone_id = data.aws_route53_zone.mxfactorial_io.zone_id

  records = [tolist(aws_acm_certificate.client_www_cert.domain_validation_options)[0].resource_record_value]
