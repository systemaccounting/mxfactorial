...
RUN USER=root cargo build \
	--manifest-path=services/auto-transact/Cargo.toml \
...
COPY --from=builder /app/target/x86_64-unknown-linux-musl/release/auto-transact /app/auto-transact

EXPOSE 10011

CMD [ "/app/auto-transact" ]
