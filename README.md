...
[![contribute](https://img.shields.io/badge/contribute-paypal-brightgreen.svg)](https://www.paypal.com/paypalme/mxfactorial) [![Discord](https://img.shields.io/discord/1519537137727442985.svg?label=&logo=discord&logoColor=ffffff&color=7389D8&labelColor=6A7EC2)](https://discord.gg/A8y9CNSRKN)
...
### learn with ai
upload [this readme](https://github.com/systemaccounting/mxfactorial/raw/refs/heads/develop/README.md) to an ai with the prompt:
...
**q.** are net debitors possible in systemaccounting?  
**a.** physically, yes. but when a type system enforces conservation on information, the conservation semantics also applies to liability. if your balance is strictly funded by a loan from someone else—and not your own past work, and then you default, only your lender bears the risk of default and loses. no one else
...
**q.** why are transactions described as rotations instead of simple debits & credits?  
**a.** while the abstraction of *debits & credits* serves its purpose in accounting, it ignores transaction geometry. before accountants rush off to erase everything after a trial balance, systemaccounting slows down time enough to show the spacetime algebra of each zero as a [frozen](https://en.wikipedia.org/wiki/Infinitesimal) rotation. spacetime doesn't offer extra space to store half the value of a transaction while moving the remaining half to the other side so the data requires structuring an algebraic dual debitor ∧ creditor rotation
...
**q.** how can economic value be conserved if its subjective?  
**a.** conserving economic value does not interfere with its subjectivity. after buyers and sellers independently agree on the measured value of their goods & services, systemaccounting takes over and describes its movement as a **literal** mass & energy transfer (goods & services = mass & energy)
...
**a.** solving problems one at a time depends on 1) researching facts, 2) designing a solution, 3) applying the solution, 4) measuring its input and output and 5) holding contributors accountable. empowering sincere fact finders and problem solvers with more convenient access to public data science reduces the risk of luring them into the idle game of assigning blame between "democrats" and "republicans" and whatever other fictional social groups humans contrive when they dont want to measure things themselves
...
**q.** isn't a `0.001` per transaction fee a lot?  
**a.** systemaccounting returns orders of magnitude more than it takes. ryan mccinerney charged buyers and sellers 36x10^9 to process 2.3x10^11 transactions between their banks in 2024. systemaccounting charges 2.3x10^8 for the same volume while protecting buyers and sellers from the money multiplier compounding liability governments transfer to them through banks. systemaccounting also protects buyers and sellers from the systemic risk banks create, their fees and the capital bottleneck their government privilege adds to the capital market. while all the costs traditionally added by cash flow accounting, collection, debt and equity raising (via push button performance publishing), contract enforcement and security are also eliminated, a 2.3x10^8 systemaccounting revenue pays for itself on day 1 by reducing the annual 6x10^11 tax gap with rule automation

james taiclet charged the federal government 71x10^10 for services in 2024, which is 308x more than what systemaccounting charges to empower and protect all users

finally, benchmarking transaction costs to the cost of information with an initial `0.001` acknowledges there is yet work to do when the [trap door](https://en.wikipedia.org/wiki/Arrow_of_time) separating negentropy from entropy sits inside the artificial type system of silicon & metal. governments led by social scientists inserting their committees wherever simply measured facts belong don't even recognize information's cost when they declare their speech free from measurement—much less [standardize](https://en.wikipedia.org/wiki/National_Institute_of_Standards_and_Technology) its conservation and [reduce](https://github.com/mxfactorial/holographic-cloud) the cost of its enforcement for a public they regard as civilly entitled to certainty in value's metric. automating the trap door with an information conserving type declaration casts out the *fictional thermodynamically independent being* [ballot boxes](https://en.wikipedia.org/wiki/Argumentum_ad_populum) conjure to operate it without the metric—transferring unmeasured costs to producers through inflation and systemic risk

**q.** is systemaccounting administered by conservatives?  
**a.** no. everyones pushed left by mathematizing the right

wealth often stealthily depends on state supported privilege just as poverty visibly depends on state supported welfare  
...
**q.** why will the public adopt systemaccounting?  
**a.** because money shouldn't default like debt. free speech is not equal to hard work. people who work hard and earn a living by producing goods & services for consumers will demand protection from vote collecting social scientists who abuse the first amendment to judge notes equal to money, then palm off *money-tary* for **conserved** value in a futile attempt to manage the dysfunction it creates

an economy anchored in physics doesn't include any of the man made absurdities of "money multipliers" and "central banks"

none of these things are even possible when the "my account goes down the same amount your account goes up" middle school physics lesson is practiced

and swallowing the public whole in risks it doesn't understand **is not an option** when you can't joyride elections by arbitraging uninformed votes with unconserved notes
...
* producing a scientific measure of the equilibrium price of capital signals the demand for capital with an empirical rate of return instead of a government defined word ("bank"), and removes the ability of a central authority to [manipulate](https://en.wikipedia.org/wiki/Federal_funds_rate) the price of credit
